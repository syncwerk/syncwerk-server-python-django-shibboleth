...
from restapi.base.sudo_mode import update_sudo_mode_ts
from restapi.profile.models import Profile
from restapi.utils.file_size import get_quota_from_string
from restapi.utils.user_permissions import get_user_role
...
class ShibbolethRemoteUserMiddleware(RemoteUserMiddleware):
    ...
    def _set_auth_cookie(self, request, response):
        from restapi.api2.utils import get_token_v1, get_token_v2
        ...
        if all(['shib_' + key in request.GET for key in keys]):
            ...
            return
        response.set_cookie('restapi_auth', request.user.username + '@' + token.key)
    ...
    def update_user_quota(self, user, user_role):
        if user.permissions.role_quota():
            ...
            syncwerk_api.set_role_quota(user_role, quota)
        else:
            return
